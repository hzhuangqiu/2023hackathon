...
auto read_native_binary(const std::string& path) {
...
int main() {
    for (auto model : {"lightgbm", "xgboost", "catboost"}) {
        auto [buffer, length] =
            read_native_binary(std::string{"native_binary_"} + model + ".txt");
        daal_data::OutputDataArchive out_data_arch{buffer, length};
        delete[] buffer;

        std::cout << "Unpack model (" << model << ")...\n";
        ...
        algo.input.set(daal_algo::classifier::prediction::model,
                       deserialized_model);
        std::cout << "Start inferring (" << model
                  << " && oneDAL with C++)...\n";
        auto begin = std::chrono::system_clock::now();
        algo.compute();
        auto end = std::chrono::system_clock::now();
        std::cout << "Inference time (" << model << " && oneDAL with C++): "
        ...
        auto num_ptr =
            pred_res->get(daal_algo::classifier::prediction::prediction);
        ...
        std::cout << "First ten results (" << model << "):\n";
